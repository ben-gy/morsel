 ...
 *     code, each convinced they are the host, seeing nobody. In Morsel the host
 *     also owns the dish — pellet spawns, swallows, respawns — so a room with two
 *     hosts is a room where the two dishes silently diverge.
 *
...
describe('host election — the incumbent keeps the room', () => {
  ...
  it('keeps the incumbent across a full table, whatever their ids', async () => {
    // Fill the room and make sure none of the three joiners takes it, including
    // the two that sort below the host.
